...
def album(raw_data):
    ...
    for row in raw_data['items']:
        ...
        album_elements = {'album_id' : album_id,'album_name': album_name,'album_url':album_url,'album_release_date': album_release_date,'album_track_duration': album_track_duration}
    ...
    if "Contents" not in response:
        ...
        for file in response["Contents"]:
            ...
            if file_key.endswith(".json"):
                resp = s3.get_object_v2(Bucket=bucket, Key=file_key)
                data = resp["Body"].read()
                obj = json.loads(data)
                raw_data.append(obj)
                raw_data_keys
